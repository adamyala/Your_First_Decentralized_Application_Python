...
# easysolc is a python wrapper for solc and is needed to compile our Solidity code
# Docs: https://github.com/brunneis/easysolc
...
# Instantiate Solc. This will use the first accessible compiler from the $PATH
# environment variable. If you need to use a custom path, you can do so by
# passing the custom path as an argument to the constructor.
# E.g. solc = Solc('/usr/local/bin/solc')
solc = Solc()
...
solc.compile('voting_v0_5_x.sol', output_dir='.')

# Obtain the web3 contract instance given the bytecode and ABI file
contract = solc.get_contract_instance(
    abi_file='Voting.abi',
    bytecode_file='Voting.bin'
)
...
contract_constructor = contract.constructor(VOTING_CANDIDATES)
...
contract_instance = eth_provider.contract(
    abi=contract.abi, # obtained from our contract's web3 instance
